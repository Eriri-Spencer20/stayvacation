...
export default function button(props) {
	...
	if (props.type === "link") {
		if (props.isExternal) {
			return (
				<a href={props.href} className={className.join(" ")} style={props.style} target={target.props === "_blank" ? "_blank" : undefined} rel={target.props === "_blank" ? "noopener noreferrer" : undefined}>
					{props.children}
				</a>
			);
		} else {
	...
	return (
		<button className={className.join("")} style={propTypes.style} onClick={onClick}>
			{props.children}
		</button>
	);
}
